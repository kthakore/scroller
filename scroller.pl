...
my $sprite = SDLx::Sprite::Animated->new(image=>'data/m.png',
	rect            => SDL::Rect->new( 0, 0, 16, 28),
	ticks_per_frame => 10,
);

$sprite->set_sequences( left => [ [ 0, 1 ],[ 1, 1 ],[ 2, 1 ],[ 3, 1 ],[ 4, 1 ] ] ,
...
my @collide_blocks = ([19,330], [40,330], [30,309], [ 120, 400 ], [141,400], [141,379] );
...
$obj->set_acceleration(
    sub {
		...
		if( $pressed->{right} ) { 
			$state->v_x( $vel_x ); 
			if( $pressed->{up} ){ $sprite->sequence('jumpr') }  else {$sprite->sequence('right'); }

		   }elsif( $sprite->sequence() eq 'right' && !$pressed->{left} ) { $sprite->sequence('stopr') };

		if( $pressed->{left} ) { 
			$state->v_x( -$vel_x ); 
			if( $pressed->{up} ){ $sprite->sequence('jumpl') }  else {$sprite->sequence('left'); }
		}elsif( $sprite->sequence() eq 'left' && !$pressed->{right} ) { $sprite->sequence('stopl') };
		if( $pressed->{up} && !$lockjump){ 
			
			$state->v_y( $vel_y ); $lockjump = 1;		
		...
		if( $collision != -1 && $collision->[0] eq 'x' )
		{
			...
			if( $state->v_x() < 0) { #moving left
				...
				$state->x( $block->[0] + 3 + $block->[2] ); #set to edge of block
			}
		}
...
$app->add_show_handler(
	sub { 
		...
		SDL::GFX::Primitives::string_color( $app, $app->w/2-100, $app->h/2, "Mario is DEAD", 0xFF0000FF) if $quit;
...
sub check_collision {
	...
	foreach ( @$blocks )
	{ 
		my $hash = { x => $mario->x, y=> $mario->y, w=> $w, h => $h, xv => $mario->v_x*0.02, yv => $mario->v_y*0.02 };
